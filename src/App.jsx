...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubActs = onSnapshot(collection(db, stageColPath('activities')), async (snapshot) => {
        ...
        const activitiesWithoutRef = data.filter(activity => !activity.activity_ref);
        const maxExistingRefSequence = data.reduce((max, activity) => {
          const sequence = parseActivityRefSequence(activity.activity_ref);
          return sequence && sequence > max ? sequence : max;
        }, 0);

        if (missingOrder.length > 0 || activitiesWithoutRef.length > 0) {
          ...
          let nextRefSequence = maxExistingRefSequence + 1;
          orderedById.forEach((activity) => {
            if (!activity.activity_ref) {
