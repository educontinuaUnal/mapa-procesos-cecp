...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeFlowsSnapshot = onSnapshot(collection(db, stageColPath('flows')), (snapshot) => {
    ...
    // Escuchar Fases
    const unsubscribePhasesSnapshot = onSnapshot(collection(db, stageColPath('phases')), (snapshot) => {
        let data = snapshot.docs.map(doc => doc.data());
    ...
    const unsubscribeActivitiesSnapshot = onSnapshot(collection(db, stageColPath('activities')), async (snapshot) => {
        let data = snapshot.docs.map(doc => doc.data());
    ...
    return () => { unsubscribeFlowsSnapshot(); unsubscribePhasesSnapshot(); unsubscribeActivitiesSnapshot(); };
